...
class Translation
  POLY = {methionine: %w(AUG), phenylalanine: %w(UUU UUC), leucine: %w(UUA UUG ), serine: %w(UCU UCC UCA UCG), tyrosine: %w(UAU UAC), cysteine: %w(UGU UGC), tryptophan: %w(UGG), stop: %w(UAA UAG UGA)}
  ...
  def self.of_rna(strand)
    ...
    until strand.length <= 0
      codon = self.of_codon strand.slice!(0,3)
